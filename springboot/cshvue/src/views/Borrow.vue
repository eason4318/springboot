<template>
  <div>
    ...
    <div style="margin: 10px 0">
      <el-button type="primary" @click="handleAdd">仪器预约 <i class="el-icon-circle-plus-outline"></i></el-button>
      <el-popconfirm
          class="ml-5"
      ...
      <!-- <el-upload action="http://localhost:9090/borrow/import" :show-file-list="false" accept="xlsx" :on-success="handleExcelImportSuccess" style="display: inline-block">
    ...
    <el-table :data="tableData" border stripe :header-cell-class-name="'headerBg'"  @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作"  width="180" align="center">
        <template slot-scope="scope" v-if="user.role === 'ROLE_ADMIN' || scope.row.user === user.username">
...
export default {
  ...
  methods: {
    ...
    del(id) {
      this.request.delete("/borrow/" + id).then(res => {
        if (res.code === '200') {
    ...
    delBatch() {
      ...
      this.request.post("/borrow/del/batch", ids).then(res => {
        if (res.code === '200') {
    ...
    exp() {
      window.open("http://localhost:9090/borrow/export")
    },
